...
export const PRIVACY_HTML = `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>Privacy Policy - Yoke</title><style>body{background:#0d1117;color:#c9d1d9;font-family:system-ui,-apple-system,sans-serif;max-width:800px;margin:0 auto;padding:2rem;line-height:1.7}h1{color:#58a6ff}h2{color:#8b949e;margin-top:2rem}a{color:#58a6ff}code{background:#161b22;padding:2px 6px;border-radius:4px;font-size:0.9em}ul{padding-left:1.5rem}li{margin-bottom:0.5rem}</style></head><body><h1>Privacy Policy</h1><p><strong>Last updated:</strong> June 2026</p><h2>What We Collect</h2><p>When you analyze a domain, we collect the domain name you submit. We do not use cookies, trackers, or fingerprinting. No accounts, no emails, no personal information.</p><h2>Rate Limiting &amp; IP Handling</h2><p>To prevent abuse, Yoke enforces per-IP rate limits. Your IP address is <strong>never stored in raw form</strong>. Instead, we immediately hash it using SHA-256 with a daily-rotating salt. The resulting hash is used only to count requests within a rate-limit window — it cannot be reversed to recover your IP address, and it changes every day.</p><p>Rate-limit records are automatically cleaned up within hours. No raw IP addresses are written to any database or log at any point in the request lifecycle.</p><h2>Anonymous Analytics</h2><p>Yoke collects anonymous, aggregated request metadata to operate the service:</p><ul><li><strong>Visitor hash</strong> — the same daily-salted SHA-256 hash used for rate limiting, for unique visitor counting</li><li><strong>Country code</strong> — derived from Cloudflare&apos;s <code>cf-ipcountry</code> header (2-letter code only, no geolocation)</li><li><strong>Client type</strong> — whether the request came from a browser, CLI, API client, or extension</li><li><strong>Request metadata</strong> — endpoint, analyzed domain, HTTP status code, response latency</li></ul><p>None of this is linked to an identity. There are no user accounts, sessions, or tracking pixels.</p><h2>Caching</h2><p>Analysis results are cached for up to 24 hours to improve performance. Cached data includes only publicly available DNS, WHOIS, SSL, and HTTP header information about the domains you analyze. Cache hits do not count against rate limits.</p><h2>BYO API Key (AI Analysis)</h2><p>Yoke&apos;s AI tab lets you bring your own <a href="https://openrouter.ai">OpenRouter</a> API key. Here&apos;s exactly how it works:</p><ul><li><strong>Storage:</strong> Your API key is saved in your browser&apos;s <code>localStorage</code> — on your device, under your control.</li><li><strong>In flight:</strong> When you run an AI analysis, your key is sent to Yoke&apos;s server as part of the request. The server passes it through to OpenRouter to fulfill the API call, then discards it. We don&apos;t log or store your key at any point — it exists in memory only for the duration of that single request.</li><li><strong>Removal:</strong> Click &quot;Remove key&quot; in the Advanced panel, or clear your browser&apos;s site data. That&apos;s it — there&apos;s nothing to delete on our side because we never stored it.</li><li><strong>Without a key:</strong> You still get 3 AI analyses per day using Yoke&apos;s shared platform key. No key required for any other feature.</li></ul><h2>Third-Party Services</h2><p>Analyses may query public APIs including DNS resolvers (Google, Cloudflare), RDAP/WHOIS registries, Shodan InternetDB, PageSpeed Insights, Have I Been Pwned, Brandfetch, and others. Each service has its own privacy policy. A full list of data sources is available at <a href="https://github.com/yokedotlol/yoke/blob/main/docs/DATA-SOURCES.md">DATA-SOURCES.md</a>. When you use a BYO key, your AI requests go to <a href="https://openrouter.ai/privacy">OpenRouter</a>, which routes to your selected model provider.</p><h2>Infrastructure</h2><p>Yoke is served through <a href="https://www.cloudflare.com/">Cloudflare Workers</a>. Cloudflare collects anonymous server-side request metrics (traffic volume, country, status codes) as part of its infrastructure. No cookies, no client-side tracking. See <a href="https://www.cloudflare.com/privacypolicy/">Cloudflare&apos;s privacy policy</a> for details.</p><h2>Data Retention</h2><p>Rate-limit hashes are cleaned up automatically within hours. Analytics data is retained indefinitely in aggregate form but contains no personal identifiers. Domain analysis results are cached for up to 24 hours. Domain scores and scan history are retained to power the <a href="/_/showcase">popular domains</a> feed and percentile rankings.</p><h2>GDPR</h2><p>Yoke does not store personal data as defined under GDPR. IP addresses are hashed before storage and cannot be reversed. No cookies, accounts, or tracking identifiers are used. If you have questions about data handling, <a href="https://github.com/yokedotlol/yoke/issues">open an issue</a>.</p><h2>Open Source</h2><p>Yoke is <a href="https://github.com/yokedotlol/yoke">open source</a> (MIT license). You can audit exactly what data is collected and how it&apos;s handled. You can even <a href="https://github.com/yokedotlol/yoke/blob/main/docs/SELF-HOSTING.md">host it yourself</a> with full control.</p><h2>Contact</h2><p>Questions? <a href="https://github.com/yokedotlol/yoke/issues">Open an issue</a> on GitHub.</p><p><a href="/">← Back to Yoke</a></p></body></html>`;
...
export const ABOUT_HTML = `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1"><title>About - Yoke</title><style>body{background:#0d1117;color:#c9d1d9;font-family:system-ui,-apple-system,sans-serif;max-width:800px;margin:0 auto;padding:2rem;line-height:1.7}h1{color:#58a6ff}a{color:#58a6ff}p{margin-bottom:1rem}code{background:#161b22;padding:2px 6px;border-radius:4px;font-size:0.9em}</style></head><body><h1>About Yoke</h1><p>Yoke is a free, open-source domain intelligence tool. Paste in a domain, get a full picture — DNS, SSL, security headers, email auth, breach history, tech stack, performance, and more. One search, 155+ signals, scored across six categories.</p><p>It's built for sysadmins, DevOps engineers, security teams, freelancers, consultants, and anyone else who needs to quickly answer "what's going on with this domain?" without juggling six different tools.</p><p>Yoke is MIT-licensed. The source is on <a href="https://github.com/yokedotlol/yoke">GitHub</a> — self-host it, fork it, contribute to it.</p><p>Feature requests, feedback, and bug reports all live on <a href="https://github.com/yokedotlol/yoke/issues">GitHub Issues</a>. ✌️</p><p><a href="/">← Back to Yoke</a></p></body></html>`;
