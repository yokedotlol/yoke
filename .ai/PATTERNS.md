...
Single source of truth for all 153 scoring signals. Every signal declares its axis, actionability, effort, fix description, and weight range. Derived constants (`NON_ACTIONABLE`, `EFFORT_MAP`, `FIX_DESC_MAP`, etc.) are exported for use across server and client.
