...
- **No raw IP storage.** Rate-limit keys use a secret-salted stable SHA-256 hash so sliding windows survive day boundaries and are pruned within hours. Operational analytics are hourly aggregate counters with no IP hash, request target, timestamp, or other per-request identifier. Raw IPs are not written to Yoke's databases.
